...
// isDuplicateTcpRemoteError 判断 sqlite 错误是否为 (type, remote_port) 部分唯一索引冲突。
//
// 约束来源：internal/storage/sqlmigrations/0001_init.up.sql 中的
// `CREATE UNIQUE INDEX idx_proxies_tcp_remote ON proxies(type, remote_port) WHERE remote_port IS NOT NULL;`
// 实证错误文本（见 proxies_test.go TestIsDuplicateNameError_DirectChecks
// "type-remote-conflict" 用例 L137）：
//
//	UNIQUE constraint failed: proxies.type, proxies.remote_port
//
// 关键字组合："UNIQUE constraint failed" + "proxies.type" + "proxies.remote_port"。
//
// T-018 B-8 修订要求：实现前应先 adversarial 单测捕获真实文本。本函数关键字
// 即据 proxies_test.go L137 已实证文本固化；测试 TestUpsertProxiesTx_DuplicateTcpRemote_TextLeak
// 进一步实证 modernc.org/sqlite 当前版本在事务内 INSERT 同样输出此文本。
func isDuplicateTcpRemoteError(err error) bool {
	if err == nil {
		return false
	}
	s := err.Error()
	return strings.Contains(s, "UNIQUE constraint failed") &&
		strings.Contains(s, "proxies.type") &&
		strings.Contains(s, "proxies.remote_port")
}

// UpsertProxiesTx 在单事务内插入 ps 数组中的全部 proxy 记录。任一失败 → 全部回滚。
//
// 限制：仅支持新建（要求每条 p.ID == 0）；调用方负责保证。
// 总条数上限校验放在 handler 层（需先 count 现有数）。
//
// 返回值：
//   - 全部成功：(insertedIDs, nil)，p.ID / p.Version=1 / p.UpdatedAt 已回填到 ps 切片中各元素。
//   - 任一冲突：(nil, conflictErr)，事务回滚，DB 完全不变。
//
// 冲突类型映射：
//   - proxies.name UNIQUE → ErrDuplicateName（沿用 T-007）
//   - (type, remote_port) 部分唯一索引 → ErrDuplicateTcpRemote（T-018 新增）
//   - 其它 sqlite 错误 → 原始 wrap
//
// 并发约束（T-018 B-5）：与 UpsertProxy 同款 —— 整段持 s.mu.Lock()：
// BeginTx 前 Lock、Commit/Rollback 后 Unlock。原因：
//   - store.go SetMaxOpenConns(1) 限制 sqlite 单连接，所有写操作必须串行化；
//   - 不持锁时事务 INSERT 与并行 UpsertProxy/DeleteProxy 共享物理连接 →
//     可能出现 "database is locked"。
//
// 与单条 UpsertProxy 不同：
//   - UpsertProxy 在新建路径**不**用事务（单 INSERT），更新路径才开事务做 version check；
//   - UpsertProxiesTx 总是开事务（批量原子性是核心）。
func (s *Store) UpsertProxiesTx(ctx context.Context, ps []*Proxy) ([]int64, error) {
	if len(ps) == 0 {
		return nil, nil
	}
	// shape 预校验（在持锁之前做，失败不影响其它 goroutine）。
	for i, p := range ps {
		if p == nil {
			return nil, fmt.Errorf("storage.UpsertProxiesTx: ps[%d] is nil", i)
		}
		if p.ID != 0 {
			return nil, fmt.Errorf("storage.UpsertProxiesTx: ps[%d] has non-zero ID (insert-only API)", i)
		}
		if err := validateProxyShape(p); err != nil {
			return nil, err
		}
	...
	// B-5：整段持 s.mu.Lock()，避免 sqlite 单连接上的事务与并行 UpsertProxy 互锁。
	s.mu.Lock()
	defer s.mu.Unlock()

	tx, err := s.db.BeginTx(ctx, nil)
	if err != nil {
		return nil, fmt.Errorf("storage.UpsertProxiesTx begin: %w", err)
	}
	// Commit 后 Rollback 是 no-op，安全 defer。
	...
	for _, p := range ps {
		cdJSON, encErr := encodeCustomDomains(p.CustomDomains)
		if encErr != nil {
			return nil, fmt.Errorf("storage.UpsertProxiesTx encode customDomains: %w", encErr)
		}
		enabledInt := 0
		if p.Enabled {
			enabledInt = 1
		...
		res, execErr := tx.ExecContext(ctx, insertQ,
			p.Name, p.Type, localIP, p.LocalPort,
			nullableInt(p.RemotePort), nullableString(cdJSON), enabledInt)
		if execErr != nil {
			// 区分冲突 sentinel；defer Rollback 自动回滚。
			if isDuplicateNameError(execErr) {
				return nil, ErrDuplicateName
			}
			if isDuplicateTcpRemoteError(execErr) {
				return nil, ErrDuplicateTcpRemote
			}
			return nil, fmt.Errorf("storage.UpsertProxiesTx insert %q: %w", p.Name, execErr)
		}
		id, idErr := res.LastInsertId()
		if idErr != nil {
			return nil, fmt.Errorf("storage.UpsertProxiesTx lastid: %w", idErr)
		}
		p.ID = id
		p.Version = 1
		ids = append(ids, id)
	...
	if err := tx.Commit(); err != nil {
		return nil, fmt.Errorf("storage.UpsertProxiesTx commit: %w", err)
	}

	// Commit 成功后逐条回填 UpdatedAt（不计入事务、不影响原子性）。
	for _, p := range ps {
		if fresh, gerr := s.getProxyLocked(ctx, p.ID); gerr == nil && fresh != nil {
			p.UpdatedAt = fresh.UpdatedAt
		}
	}
	return ids, nil
